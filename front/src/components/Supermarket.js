            ...
            <Grid container spacing={2} sx={{m:5}}>
                <Grid item xs={8}>
                    <IconButton item xs={4} color="primary" onClick={() => setClicked(true)} aria-label="add to shopping cart">
                        Cart < ShoppingCartOutlinedIcon color="secendery" sx={{ml:2}}/>
                    </IconButton>
                </Grid>
                <Grid item xs={12}>
                    <Typography align="center" component="div" variant="h4">Supermarket</Typography>
                </Grid>
                <Grid item xs={12} container sx={{mt:4}}>
                    {productList.map((product,index) => {
                        return (
                            <Grid item xs={6} container direction="row" key={index}>
                                <Grid item xs={8}> 
                                    <Product 
                                    title={product.title}
                                    price={product.price}
